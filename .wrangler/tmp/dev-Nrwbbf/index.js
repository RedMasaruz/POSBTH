...
// .wrangler/tmp/bundle-Bxk7Sg/checked-fetch.js
...
// .wrangler/tmp/bundle-Bxk7Sg/strip-cf-connecting-ip-header.js
...
var KVError = /* @__PURE__ */ __name(class _KVError extends Error {
  static {
...
var MethodNotAllowedError = /* @__PURE__ */ __name(class extends KVError {
  static {
...
var NotFoundError = /* @__PURE__ */ __name(class extends KVError {
  static {
...
var InternalError = /* @__PURE__ */ __name(class extends KVError {
  static {
...
app.use("/api/*", cors());
var generateId = /* @__PURE__ */ __name((prefix) => {
  const datePart = (/* @__PURE__ */ new Date()).toISOString().replace(/[-:T.]/g, "").slice(2, 14);
  const randomPart = Math.floor(Math.random() * 1e3).toString().padStart(3, "0");
  return `${prefix}${datePart}${randomPart}`;
}, "generateId");
var errorResponse = /* @__PURE__ */ __name((c, message, status = 400) => {
...
app.post("/api/auth/login", async (c) => {
  try {
    ...
    if (!username || !password)
      return errorResponse(c, "Missing credentials", 400);
...
app.get("/api/analytics", async (c) => {
  try {
    ...
    const queryParams = params.length > 0 ? params : void 0;
    const bindQuery = /* @__PURE__ */ __name((q) => {
    ...
    const kpiQuery = `
        ...
        `;
    const kpiResult = await bindQuery(kpiQuery).first();
    const trendQuery = `
            SELECT 
    ...
    const totalProductsQuery = `
        ...
        ${joinFilter}
    `;
    const totalProductsResult = await bindQuery(totalProductsQuery).first();
...
app.post("/api/products", async (c) => {
  try {
    const body = await c.req.json();
    if (!body.name || !body.sku || body.price === void 0) {
    ...
    if (existing)
      return errorResponse(c, "SKU already exists", 400);
    await c.env.DB.prepare(
...
app.put("/api/products/:id", async (c) => {
  try {
    ...
    const body = await c.req.json();
    if (!id)
      return errorResponse(c, "Product ID missing");
    const updates = [];
    const values = [];
    const fields = [
      "name",
...
app.post("/api/orders", async (c) => {
  try {
    ...
    if (body.userId) {
      const user = await c.env.DB.prepare("SELECT role FROM users WHERE id = ?").bind(body.userId).first();
      if (user) {
        if (user.role === "dealer")
          priceField = "price_dealer";
        if (user.role === "dealer_vip" || user.role === "vip")
    ...
    for (const item of items) {
      if (item.quantity <= 0)
        continue;
      const product = await c.env.DB.prepare("SELECT * FROM products WHERE id = ?").bind(item.productId).first();
      if (!product) {
    ...
    const setting = await c.env.DB.prepare("SELECT value FROM settings WHERE key = 'discount_rate'").first();
...
app.delete("/api/orders/:id", async (c) => {
  try {
    ...
    const statements = [];
    for (const item of items) {
      const product = await c.env.DB.prepare("SELECT name, stock FROM products WHERE id = ?").bind(item.product_id).first();
...
// .wrangler/tmp/bundle-Bxk7Sg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Bxk7Sg/middleware-loader.entry.ts
